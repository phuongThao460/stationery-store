import { CHAT_LIEU_Model, Update } from "../models/CHAT_LIEU_Model.js";
...
export const Update_Chat_Lieu = async (req, res) => {
  try {
    const chat_lieu = req.body;
    const rs = await Update(chat_lieu);
    res.json(rs);
  } catch (err) {
    console.log(err);
    res.json(err);
