...
import { Update } from "../models/TTKH_Model.js";
...
export const Get_address = async (req, res) => {
  try {
    ...
    const address =
      phuong.phuong_xa + ", " + quan.quan_huyen + ", " + tp.ten_thanh_pho;
...
export const Get_TTKH_By_ID = async (req, res) => {
  ...
  try {
    ...
    const ttkh = await TT_KH_Model.findById(ttkh_id).populate({
      path: "id_phuong",
      select: "phuong_xa",
      populate: {
        path: "id_quan",
...
export const Update_TTKH = async (req, res) => {
  /*
	Update ttkh
  ...
  try {
    const new_ttkh = req.body;
    const rs = await Update(new_ttkh);
